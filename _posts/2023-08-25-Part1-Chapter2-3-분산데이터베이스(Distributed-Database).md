---
layout: post
title: Part1 Chapter 2-3 분산데이터베이스 (Distributed Database)
author: admin
...
- 분산 데이터베이를 사용하는 고객은 시스템이 네트워크로 분산되어 있는지의 여부를 인식하지 못하면서, 자신만의 데이터베이스를 사용하는 것처럼 사용할 수 있다. 이처럼 데이터베이스는 <span style="color:tomato">투명성</span>을 제공해야 한다.
...
| 장점 | 설명 |
| --- | --- |
| 데이터베이스 <span style="color:tomato">신뢰성과 가용성</span>이 높다. | 분산 데이터베이스는 여러 시스템에 데이터를 분산하여 저장하므로, 하나의 시스템에 장애가 발생해도 다른 시스템에서 데이터를 제공할 수 있어 신뢰성과 가용성이 높다. |
| <span style="color:tomato">빠른 응답 속도</span> | 병렬 처리를 통해 분산 데이터베이스는 빠른 응답 속도를 제공할 수 있다. 여러 시스템에서 동시에 작업을 처리하기 때문에 처리 성능을 향상시킬 수 있다. |
| <span style="color:tomato">용량 확장 용이성</span> | 새로운 시스템을 추가하여 분산 데이터베이스를 확장하는 것은 비교적 간단하다. 즉, 시스템 용량을 필요에 따라 쉽게 조정할 수 있다. |

**단점:**

| 단점 | 설명 |
| --- | --- |
| <span style="color:tomato">관리와 통제</span>의 어려움 | 분산 데이터베이스는 여러 네트워크로 구성되어 있기 때문에 관리와 통제가 복잡해질 수 있다. 다양한 시스템 간의 일관된 관리 및 모니터링을 위한 체계적인 접근 방식이 필요합니다. |
| <span style="color:tomato">보안 관리</span>의 어려움 | 분산된 환경에서 보안 관리는 복잡하다. 다양한 시스템 간의 접근 제어, 사용자 인증 및 권한 부여 등 보안 요구사항을 충족하기 위해서는 체계적인 보안 전략과 메커니즘이 필요하다. |
| <span style="color:tomato">데이터 무결성 관리</span>의 어려움 | 여러 개의 동일한 데이터 사본들 사이의 일관성 유지와 갱신 문제 등으로 인해 데이터 무결성 관리가 어려울 수 있다. 동기화 문제나 중복 업데이트 등으로 인한 일관성 유지가 중요하다.

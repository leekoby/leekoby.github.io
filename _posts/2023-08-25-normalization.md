...
<br/>
 ...
 |제3정규화(3NF)|- 기본키를 제외한 **`칼럼 간에 종속성을 제거`**한다. 즉, **<span style="color:#ff6600">이행 함수 종속성을 제거</span>**한다.|
 |BCNF|- 기본키를 제외하고 **`후보키`**가 있는 경우, **`후보키가 기본키를 종속시키면 분해`**한다. |
 |제4정규화(4NF)| - **`여러 칼럼들이 하나의 칼럼을 종속시키는 경우`** 분해하여 **`다중값 종속성을 제거`**한다.|
...
- 중복속성에 대한 분리가 1차 정규화의 대상이 되며, 로우 단위의 중보도 1차 정규화의 대상이 되지만 칼럼 단위로 중복되는 경우도 1차 정규화의 대상이다. 
...
> 
>**<span style="color:#ff6600">`기본키가 2개 이상의 칼럼`으로 이루어진 경우에만 발생</span>**한다.
{:.prompt-info}

>**<span style="color:#3366ff">부분 함수 종속성이란?</span>** 
>
>**<span style="color:#ff6600">기본키를 제외하고 칼럼 간에 종속성이 발생</span>**하는 것
>
{:.prompt-info}
...
>
>기본키를 제외하고 **`칼럼 간에 종속성이 발생하는 것`**<br/>
{:.prompt-info}
